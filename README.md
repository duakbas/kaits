...
minutes" is a fact and "killed after 47 minutes with the socket still open" is
a diagnosis. Each field answers a question that would otherwise cost another
day of waiting:

- **the verdict** — a goodbye is *not* proof the user closed it. KaiOS
...
The keepalive is gone. It was an inaudible tone played while the app was out
of sight, on the theory that the platform counts an app making sound as busy
and moves it up the priority list — which is how a music player survives
backgrounding. It was built, tuned, made silent on headphones, rewritten to
stop clicking, and then tested on the device with the switch both ways.

**It does not work, and the reason is not subtle.** YouTube kills the app either
way, in about a minute. YouTube Shorts takes down the phone's own UI as well.
The system is reclaiming everything it can reach, and an app politely raising
its priority is not going to be spared from that. Reopening afterwards is the
correct response, not a workaround.

So it was removed rather than left switched off: it cost 480 KB of package for
a 30-second audio fallback, a settings row on a 240-pixel screen, two config
knobs, and a standing invitation to wonder whether turning it on would help.
`git log` has it if the question ever comes back.
