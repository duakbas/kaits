...
## Before you start: the session is a file

`wa-session.db` **is** your linked-device registration. Copying it moves the
session; it doesn't clone it. Two daemons running the same file will fight over
the same WhatsApp device slot and at least one will be logged out.

So: **stop the local daemon before starting the remote one**, and don't leave a
stray copy running.
...
Both, or you keep the account and lose the history.
